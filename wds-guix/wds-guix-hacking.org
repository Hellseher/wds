...
# Modified  : <2020-4-14 Tue 17:22:37 BST>
# Author    : Sharlatan
...
- The hole system is configured from one single point, it means there is no
  ...
  in Guile Scheme file which includes partitioning, timezone, locales, users,
  groups, system-wide package(s), system-wide services, kernel, kernel parameters etc.
  - ~guix system --help~
...
#+BEGIN_EXAMPLE
(guix)$ sudo su
(guix)# sed -i -e 's/[;]*(service openssh-service-type)/(service openssh-serivce-type)/'
