...
# Modified : <2020-02-11 Tue 00:48:52 GMT> Sharlatan
# Author   : sharlatan
# Synopsis : <>
...
#+end_example
** Colon [:]
- keyword :: is a part of optional function parametes
: package:symbol
: package::symbol
It allows to access symbols in other package(s).
** Comma [,]
It used in quasiquoting to enable embanded Lisp code.
...
| s. culon       | # | : | ~#:SYMBOL~ | determine uninterned symbol  |
| s. minuse      | # | - | ~#-FOO~    | read-time conditionalization |
...
Find all avalable package(s) in current image:
...
- system deffination :: file name.asdf ~(defsystem "name")~
* Glosary
