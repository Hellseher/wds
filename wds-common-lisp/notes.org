...
# Modified  : <2020-02-11 Tue 00:58:31 GMT> Sharlatan
# Author    : Sharlatan
# Synopsis  : <>
...
presented in your configuration just put you project with ~asd~ file
...
When you put your project to one of the locations it becomes available both for =quicklisp= and =asdf=.

Quicklisp takes care about all ~:depens-on :SYSTEMNAME~ statements in
~asd~ file and download them from it's registry which is updated once
per month.
#+begin_example
...
other package manager just load your project with =asdf= itself and
